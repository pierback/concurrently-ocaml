module Output_color = Concurrentlyocaml.Output_color

let test_prefix_styles_parse_chalk_parts () =
  match Output_color.prefix_styles ~command_index:2 "red.bold.bgBlue" with
  | Error _ -> assert false
  | Ok styles ->
      assert (List.length styles = 3);
      assert ((List.nth styles 0).Output_color.open_codes = [ 31 ]);
      assert ((List.nth styles 1).Output_color.open_codes = [ 1 ]);
      assert ((List.nth styles 2).Output_color.open_codes = [ 44 ])

let test_prefix_styles_reject_unknown_part () =
  match Output_color.prefix_styles ~command_index:0 "red.nope" with
  | Ok _ -> assert false
  | Error part -> assert (part = "nope")

let test_auto_colors_are_bounded_by_palette () =
  match Output_color.prefix_styles ~command_index:8 "auto" with
  | Error _ -> assert false
  | Ok styles ->
      assert (List.length styles = 1);
      assert ((List.nth styles 0).Output_color.open_codes = [ 36 ]);
      assert ((List.nth styles 0).Output_color.close_codes = [ 39 ])

let test_function_style_colors_fall_back_to_reset () =
  match Output_color.prefix_styles ~command_index:0 "rgb(1,2,3)" with
  | Ok _ -> assert false
  | Error part -> assert (part = "rgb(1,2,3)");
  match Output_color.prefix_styles ~command_index:0 "ansi256(123)" with
  | Ok _ -> assert false
  | Error part -> assert (part = "ansi256(123)")

let () =
  test_prefix_styles_parse_chalk_parts ();
  test_prefix_styles_reject_unknown_part ();
  test_auto_colors_are_bounded_by_palette ();
  test_function_style_colors_fall_back_to_reset ()
