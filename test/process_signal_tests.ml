...
let test_known_signal_numbers_and_labels () =
  ...
  assert (Process_signal.number (Run_policy.Named_signal "hup") = Ok Sys.sighup);
  assert (
    Process_signal.number (Run_policy.Named_signal "SIGQUIT")
    = Ok Sys.sigquit);
  assert (
    Process_signal.number (Run_policy.Named_signal "iOt") = Ok Sys.sigabrt);
  assert (
  ...
  assert (Process_signal.kill_label (Run_policy.Named_signal "term") = "SIGTERM")
